...
targets.floor_space_per_resident = []; % Disabled: retain the paper construction-cost normalization.
...
targets.height_gap = 0; % Baseline: no height restriction. Input is a FRACTION from 0 to 1.
% Only calibrate this moment under the paper parameterization: T is tied to its height scale.
% Example: set targets.height_gap = 0.5 for a 50% height gap (0.1 for 10%).
...
targets.wage = []; % Average ANNUAL wage per urban worker; [] retains model monetary units.
...
SAVE_RESULTS(fullfile(toolkit,'outputs','Paper'),baseline,counterfactual,statistics,figures); % Refresh Figures (PNG/PDF), Tables (TEX/CSV/XLSX), and full MAT results.
