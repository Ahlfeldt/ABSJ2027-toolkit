...
fields = {'N','urban_share','GDP','wage_bill','y','total_urban_area','AREA','density','x0','x1','F_C','F_R','F','p_R','p_C','CC','commuting_income_loss','DD','HA','U_bar','V','LV','floor_space_per_resident','housing_expenditure','height_gap_pct'}; % Ordered model outcomes.
...
unit = ["people";"fraction";units.flow;units.flow;units.flow+" / worker";units.area;units.area;units.density;units.distance;units.distance;units.floor_area;units.floor_area;units.floor_area;units.rent;units.rent;"index";units.flow+" / resident";"index";"index";"utility units";"utility units";units.flow;units.floor_per_resident;units.flow+" / resident";"%"]; % Use physical spatial units only when a geographic-area target identifies their scale.
if units.spatial
