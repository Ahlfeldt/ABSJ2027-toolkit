...
\end{abstract}
\vfill
\noindent\small Implementation companion. Monetary and spatial quantities are in model units unless the corresponding empirical targets supply currency and physical spatial scales. The toolkit covers illustrative city experiments; it does not reproduce the complete empirical pipeline.
\end{titlepage}
...
The economy consists of a circular city and its rural hinterland. Let $u\in\{R,C\}$ denote residential and commercial use, and let $x\geq0$ denote distance in model units from the centre. Available land in a radial interval is $\dd L(x)=2\pi\ell x\dd x$. The equilibrium state is wage $y$, urban utility $\Ubar$, and density $\dens$. Regional population $\Nbar$ and the rural-utility primitive $\Utilde$ govern urban population:
\begin{align}
...
$\tau^R,\tau^C$ & \code{tau_R}, \code{tau_C} & Spatial decay per model distance unit: $0.016$, $0.014$.\\
$\underline x^R,\underline x^C$ & \code{x_core_R}, \code{x_core_C} & Flat core radii: $1$, $1$ model distance units.\\
...
$x_{1,\max}$ & \code{x1_max} & Maximum urban radius: $\infty$ model distance units.\\
...
$\dens$ & \code{density} & Urban population per developable model area unit unless physical area is targeted.\\
$N$ & \code{N} & Endogenous urban population.\\
$x_0,x_1$ & \code{x0}, \code{x1} & CBD boundary and residential fringe (model distance units unless physical area is targeted).\\
...
\noindent Under the paper parameterization, monetary and spatial levels are in model units: distances, areas, floor space, density denominators, and rent denominators do not represent kilometres or square metres. A wage target identifies annual currency units. A total-urban-area target identifies kilometres and square kilometres and thereby supports square-metre reporting for floor-space quantities and rents. Consequently, either physical housing target requires the area target.
\clearpage
...
\end{align}
The last integral extends to the fixed numerical radius, 100 model distance units by default. This radius is distinct from the urban growth boundary and should remain unchanged across baseline and counterfactual.
...
\noindent The grid defaults to $R_{\mathrm{grid}}=100$ and $h=0.01$ model distance units. The replication uses $\delta$, rather than $h$, in its quadrature weights. The toolkit preserves this convention. Developable cell area is $\ell G_i$; reported urban area remains $\pi\ell x_1^2$.
...
Run \code{MASTER_PAPER.m} or \code{MASTER_EMPIRICAL.m} in \code{MATLAB/scripts}. The user-facing inputs are baseline parameters and fundamentals, empirical moments, and optional counterfactual changes. The execution sequence is
\begin{equation*}
...
\begin{verbatim}
changes = struct();
changes.set.x1_max = 25;    % Maximum urban radius.
...
The optional \code{targets.floor_space_per_resident} targets residential square metres per model resident; its default is empty (disabled). It requires \code{targets.total_urban_area}, which identifies the physical spatial scale. Without that area target, the starting costs $c^R_0=c^C_0=150$ imply about $0.000956$ model floor-space units per resident, not 956 square metres. A common multiplier $k$ sets $c^R=kc^R_0$ and $c^C=kc^C_0$, preserving their initial ratio. An empty target retains both supplied costs.
...
\code{MASTER_PAPER.m} preserves the paper's parameter values and the original 0.01 model-distance-unit radial grid. Population, urbanization and height-gap inversion remain active, while wage conversion and additional physical targets are empty. Its output table explicitly labels all spatial quantities and rent denominators as model units. It saves figures and tables under \code{MATLAB/outputs/Paper}.
...
Both scripts reset their input structures, accept optional counterfactual changes and contain no function definitions. The separate output directories retain both examples when users switch between them. The empirical example is an alternative quantification, not a claim that the paper's original parameterization matches those additional physical moments.
